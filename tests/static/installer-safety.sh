...
rg -q 'imagemagick' "$arch_installer" "$arch_repair" || fail 'Arch scripts must provide an ICO decoder for the vendor tray icon'
rg -q 'ImageMagick' "$installer" "$repair" || fail 'Fedora scripts must provide an ICO decoder for the vendor tray icon'
rg -q 'ICO:.*PNG32:' "$arch_installer" "$installer" "$arch_repair" "$repair" || fail 'install and repair scripts must decode the vendor ICO tray icon to a real PNG'
rg -q 'ID:-\} == omarchy' "$arch_installer" "$arch_repair" || fail 'Arch scripts must detect Omarchy for the tray-theme bridge'
rg -q 'Yaru-gray' "$arch_installer" "$arch_uninstaller" "$arch_repair" || fail 'Arch scripts must manage the Yaru-gray bridge lifecycle'
rg -q 'managed by minitela-linux-compat: Yaru-gray' "$arch_installer" "$arch_uninstaller" "$arch_repair" || fail 'Arch scripts must only touch a managed Yaru-gray bridge'
rg -q 'SUDO_USER' "$arch_installer" "$arch_uninstaller" "$arch_repair" || fail 'Arch scripts must resolve the desktop user for the user-level bridge'
